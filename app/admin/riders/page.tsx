...
export default function AdminRidersPage() {
  ...
  useEffect(() => {
    ...
    const loadRiders = async () => {
      try {
        const data = (await apiFetch("/customers/admin/riders")) as AdminRider[];
        setRiders(data);
      } catch (e) {
        const message = e instanceof Error ? e.message : "Failed to load riders";
        setError(message);
      } finally {
        setLoading(false);
      }
      ...
      <div className="mx-auto max-w-6xl">
        <div className="mb-6 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-blue-700/60">All users with role: rider</p>
          </div>
          <Link
            href="/admin"
        ...
        <div className="overflow-hidden rounded-3xl border border-white bg-white shadow-2xl shadow-blue-100/40">
          {loading ? (
            ...
            <div className="p-8 text-sm font-semibold text-blue-500">No riders found.</div>
          ) : (
